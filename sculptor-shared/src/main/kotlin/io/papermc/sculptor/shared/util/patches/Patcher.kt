...
data class PatchFailure(override val patches: List<Path>, override val failures: List<PatchFailureDetails>) :
    PatchResult {
    constructor(patch: Path, details: String) : this(listOf(patch), listOf(PatchFailureDetails(patch, details)))
}

data class PatchFailureDetails(val patch: Path, val details: String)
